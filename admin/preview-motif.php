...
<?php 
    $query = "SELECT * FROM motif INNER JOIN occasion on occasion.occasion_id = motif.occasion_id";
	$getOccasion = getdata_inner_join($query);
?>
<?php 
...
<?php foreach ($getOccasion as $key => $value):?>
    ...
    <td><?php echo $value->motif_decs?></td>
