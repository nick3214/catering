...
.ui-datepicker-calendar ,tr ,td ,th{
...
<div class="modal fade" id="additional" role="document">
        ...
        <table class="table table-bordered">
          ...
          ?>
                <tr>
                  ...
                  <td> 
                  <input type="text" name="qty" id="qty-<?php echo $row['id'];?>" size="2" placeholder="1">&nbsp;
                    <span id="add" 
  ...
  <div class="row">
    ...
    <form method="get">
      ...
      <h4><i class="fa fa-plus"></i> Food Order</h4><hr>
      <div class="col-md-3">
        <select class="form-control " name="meal_type" id="search">
        <?php 
       ...
       <select class="form-control" name="category" id="category">
            <?php
             $sql = $dbcon->query("SELECT * FROM food_categories") or die(mysqli_error());
              while($row = $sql->fetch_assoc()){
              if(isset($_GET['search_btn'])){
                  if($_GET['category'] == $row['f_id']){
       ...
       <select class="form-control" name="sub_category" id="sub_category">
            <?php
             $sql = $dbcon->query("SELECT * FROM sub_category") or die(mysqli_error());
              while($row = $sql->fetch_assoc()){
              if(isset($_GET['search_btn'])){
                  if($_GET['sub_category'] == $row['sub_id']){
  ...
  <p></p>
  <button class="btn btn-primary col-md-1"  id="btn" name="search_btn"><i class="fa fa-search"></i> </button>
  </form>

  <div class="col-md-1">
   <a href="#" class="btn btn-primary" data-target="#additional" data-toggle="modal"><i class="fa fa-plus"></i>Extras</a>
  </div>
  <div class="col-md-12">
...
<hr>
          ...
          <?php 
           if(isset($_GET['search_btn'])){
            ...
            where 
            food_items.menu_type ='".$_GET['meal_type']."' && 
            food_categories.f_id ='".$_GET['category']."' &&
            sub_category.sub_id = '".$_GET['sub_category']."'
            ";
          ...
          <?php while($row = $sql->fetch_assoc()):?>
                <tr>
                  ...
                  <input type="text" name="qty" id="qty-<?php echo $row['item_id'];?>" size="2" placeholder="10">&nbsp;
                    <span id="add" 
    ...
    <div class="col-md-5">
      <h4><i class="fa fa-shopping-cart"></i> Food Order Cart</h4><hr>
      <table id="example3" class="table table-bordered">
                ...
                <tbody>
          <?php 
          $sql = $dbcon->query("SELECT * FROM paluto_reservation_package WHERE paluto_name = '".$user['user_email']."'&& paluto_status=1 && paluto_method = 'meal'") or die(mysqli_error());
          ?>
  ...
  <hr>
  <?php 
    $total = $dbcon->query("select sum(paluto_total_price) as total_price from paluto_reservation_package WHERE paluto_name = '".$user['user_email']."'&& paluto_status=1 &&paluto_method='meal'");
  ...
  <input type="hidden" value="<?php echo number_format($total_price->total_price,2);?>" id="hidetotal">
  </div>
  </div>
  <button id="proceed" class="btn btn-info col-sm-12" name="proceed">Proceed to Checkout</button>
...
<script>
  ...
  $(document).ready(function(){
    $("#search").change(function(){
      var option = $(this).val();
      $.ajax({
        url: "paluto_meal_type.php",
        method:"post",
        data: "option="+option,
        dataType:"json",
        success:function(data){
          for(var i =0 ; i< data.length; i++){
            $("#search").append("<option>"+data.option+"</option>")
          }
        }
      })
    })
