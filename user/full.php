...
if(isset($_POST['save_button']) && isset($_GET['edit']) == '1'){
  ...
  if($photo != ""){
    if($suggest_total == '0'){
     ...
     $update = $dbcon->query("UPDATE packages SET package_code='$package_code', package_name='$package_name', package_desc='$package_desc',package_type='$package_type',total_price='$suggest_total', package_photo = 'photo.jpg' WHERE code = '".$_GET['code']."'") or die(mysqli_error());
     move_uploaded_file($_FILES["photo"]["tmp_name"],"../images/". $_FILES["photo"]["name"]);
    }
  }else{
    if($suggest_total == '0'){
...
<?php if(isset($msg)):?><div class="alert alert-danger"><?php echo $msg;?></div><?php endif;?>
            ...
            <div class="row" style="padding:5px;">
                <div class="col-md-8">
                  <h4><i class="fa fa-list"></i> Create Package Customize Package</h4>
                  <hr>
                 <form method="get">
  ...
  <select class="form-control" name="search_field">
    ...
    <?php if(!empty($r)):?>
      <?php foreach ($r as $key => $value):?>
        <option value="<?php echo $value->f_id?>"><?php echo $value->f_name?></option>
      <?php endforeach;?>
      <?php else:?>
...
<?php if(isset($_GET['search_btn'])):?>
          ...
          $sql = $dbcon->query("SELECT * FROM food_items 
            ...
            INNER JOIN food_categories on food_categories.f_id = food_items.item_category WHERE food_items.item_category = '".$_GET['search_field']."'") or die(mysqli_error());
          ?>
...
?> 
              ...
              <tr>
                ...
                <tr>
                  ...
                  <?php endforeach;?>
                   ...
                   <td></td>
                  <td>
                    <input type="hidden" name="photo" class="form-control" value="photo.jpg">
                  </td>
            ...
            <tr>
              ...
              <hr>
               ...
               <center>
              <a href="customize.php" class="btn btn-danger"><i class="fa fa-arrow-left"></i> Back to list</a>
...
function sum() {
  ...
  var result = parseFloat(result2) + (parseFloat(textone) * parseFloat(texttwo)) + parseFloat(textthree);
  if (!isNaN(result)) {
