...
$max = single_get("*","setting_id","site_settings","3");
        $result = $dbcon->query("SELECT * FROM reservations WHERE reservation_status != 'Draft' AND reservation_status != 'Cancelled'") or die(mysqli_error());
        $events = array();
        while ($row = $result->fetch_array()) {
            ...
            $e['end'] = "";
            $e['color'] = $mycolor;
            //$e['url'] = 'http://www.catering-erviems.co/user/other-reservation.php?tcode='.$row['tcode'].'&tab=1';
            //$e['allDay'] = false;
