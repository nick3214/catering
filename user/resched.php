...
if(isset($_POST['save_button'])){
  ...
  $reason = $_POST['reason'];
  $resched_date = date("Y-m-d",strtotime($_POST['resched_date']));
  $resched_time = filter($_POST['resched_time']);
  ...
  $notifSQL = array(
      "n_logs"=>"Customer: ".$_SESSION['full_name']." has requested to reschedule his/her event on ".$resched_date." / ".$resched_time." Trasaction Code ".$tcode."",
      "user_id"=>$_SESSION['user_id']
  );
  insertdata("notifications",$notifSQL);
  insertdata("cancel_resched",$insertQuery);
  header("location: index.php");
}
       ...
       <div class="container">
        ...
        <div class="box box-info" style="width:97%;">
            <div class="box-header">
              <h4 class="box-title"><i class="fa fa-calendar"></i> Reschedule Reservations 
...
<form method="post">
 ...
 $dateToday = strtotime(date("Y-m-d"));
  if($myDate > $dateToday):
  ?>
  <p>You may check available schedule here.<a href="#" onclick="return popitup('sample.php')"><h3>Click Here</h3></a></p>
  <strong>Date you want to reschedule</strong><br>
  <input type="text" class="form-control" name="resched_date" id="datepicker">
  <br>
  <strong>Time you want to reschedule</strong>
  <input type="time" class="form-control" name="resched_time">
  ...
  <?php else:?>
    <div class="alert alert-danger">You are not allowed to reschedule 1 month before the actual reservation.</div>
    <?php endif;?>
    <a href="reservations.php" class="btn btn-default"><i class="fa fa-arrow-left"></i> Back</a>
  </center>
</form>
<?php else:?>
<div class="alert alert-danger">You are not allowed to reschedule your reservation 1 week before the actual event</div>
<br>
<a href="reservations.php" class="btn btn-default"><i class="fa fa-arrow-left"></i> Back</a>
<?php endif;?>
            </div>
